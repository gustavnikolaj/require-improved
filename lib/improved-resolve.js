...
module.exports = function improvedResolve(
  ...
  if (request[0] !== "." && !path.isAbsolute(request) && !isBuiltIn(request)) {
    packageJson = packageJson || findPackageJson(parent.filename);

    let filteredRequest = request;
    let restModuleName = null;

    const moduleNameMatches = filteredRequest.match(/^([^/]*)\/(.*)$/);
    if (moduleNameMatches) {
      filteredRequest = moduleNameMatches[1];
      restModuleName = moduleNameMatches[2];
    }

    const { moduleVersion } = findVersionInPackageJson(
      filteredRequest,
      packageJson
    );

    let resolvedModulePath = resolveModuleName(
      moduleDirectory,
      request,
      moduleVersion
    );
    ...
    if (restModuleName) {
      resolvedModulePath += "/" + restModuleName;
    }
